...
public class AdamOptimizer extends Optimizer {
    ...
    /**
     * Performs a parameter update using the Adam optimization algorithm.
     * This method maintains first and second moment estimates for each parameter.
     *
    ...
    public void update(Tensor[] parameters, Tensor[] gradients) {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            for (int j = 0; j < paramData.length; j++) {
                ...
                paramData[j] -= learningRate * mHat / ((float)Math.sqrt(vHat) + epsilon);
            }
